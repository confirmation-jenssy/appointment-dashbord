...
from config import COLUMN_IDS, CONFIRMED_STATUSES, TOTAL_LEAD_STATUSES
...
def parse_meeting_date(date_string):
    ...
    date_formats = ["%Y-%m-%d %H:%M", "%Y-%m-%d", "%m/%d/%Y", "%d/%m/%Y"]
...
def build_report(items):
    ...
    for item in items:
        ...
        if same_day_status == "SAME DAY" and status in CONFIRMED_STATUSES:
            report["same_day"] += 1
...
def create_campaign_counts():
    ...
    return {
        ...
        "worked": 0,
        "left": 0,
        "today": {"10-12": 0, "1-3": 0, "4-6": 0, "7-8": 0},
        "tomorrow": {"10-12": 0, "1-3": 0, "4-6": 0, "7-8": 0},
    }
...
def build_appointment_counts(items):
    ...
    for item in items:
        ...
        # TOMORRROW COUNTS
            ...
            if confirmation == "":
                add_time_bucket(counts[campaign]["tomorrow"], dt.hour)

    for campaign in counts.values():

        campaign["left"] = campaign["total"] - campaign["worked"]
